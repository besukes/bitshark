...
//alpha usually starts at -99999 , int alpha = -99999; //Current best evaluation
//alpha - black eval , beta - white eval
...
Moves move_algorithm(GameStruct * game , CorPiece turn , int depth , int ai_level , int alpha , int beta){
    ...
    Moves ret = {.move = positional_best_move , .move_evaluation = alpha};
    return ret;
}