...
void loadTexturasPieces(GUISettings * settings ,SDL_Texture * chessPieces[12],SDL_Renderer * sdl_renderer){
    for(int i=0;i<12;i++){
        char str[30];
...
void loadAssets(GUISettings * settings , SDL_Renderer * sdl_renderer){
    loadTexturasPieces(settings,settings->textures.chessPieces,sdl_renderer);
    settings->textures.logo = IMG_LoadTexture(sdl_renderer,"assets/logo/bitshark");
    settings->textures.tabTexture = IMG_LoadTexture(sdl_renderer,"assets/tabuleiro/tab_1.png");
    settings->textures.arrow = IMG_LoadTexture(sdl_renderer,"assets/misc/triangle_orange.png");
